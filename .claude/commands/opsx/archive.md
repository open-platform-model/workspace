---
name: "OPSX: Archive"
description: Route to a repo and archive a completed change in its OpenSpec workspace
category: Workflow
tags: [workflow, archive, experimental, router]
---

Archive a completed change in the experimental workflow, in the repo that owns it.

**Input**: Optionally a repo and/or change name (e.g., `/opsx:archive add-auth`, `/opsx:archive core add-auth`). If omitted, infer from conversation context; if vague or ambiguous you MUST prompt for available changes in the resolved repo.
...
1. **Resolve the repo.** In order of precedence:
   - An explicit repo given as the first argument (`/opsx:archive <repo> ...`) or as a leading path segment.
   ...
   Only repos with a `<repo>/openspec/` directory **and** a `<repo>/.claude/skills/openspec-archive-change/SKILL.md` qualify. Discover these from the filesystem on every run; do not hardcode the list. A repo with an `openspec/` directory but no skill is not a valid target: say so and stop.
...
3. **Execute the repo-local skill.** Read `<repo>/.claude/skills/openspec-archive-change/SKILL.md` and follow it exactly as if it had been invoked, passing along the remaining arguments (the change name). The root Skill tool cannot see repo-local skills, so load the file directly.

4. **State which repo and skill ran** at the start of your reply (`Repo: core, skill: openspec-archive-change`).
